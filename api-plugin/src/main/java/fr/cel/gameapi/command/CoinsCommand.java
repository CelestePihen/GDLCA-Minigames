...
public class CoinsCommand extends AbstractCommand {
        ...
        if (args.length == 0) {
            ...
            if (args.length == 1) {
                if (sender instanceof Player player) {
                    ...
                    sender.sendMessage(GameAPI.getPrefix() + "Vous n'êtes pas un joueur...");
                }
                return;
            }
    ...
    private void sendHelp(CommandSender sender) {
        sender.sendMessage(" ");
        sender.sendMessage(ChatUtility.format("[Aide pour les commandes du sytème de pièces]", ChatUtility.GOLD));
    ...
    private void sendCoinsMessage(CommandSender player, Player target) {
        ...
        if (coins < 2) {
            sendMessageWithPrefix(player, "Le joueur " + target.getName() + " a " + coins + " pièce");
        } else {
            sendMessageWithPrefix(player, "Le joueur " + target.getName() + " a " + coins + " pièces");
        }
    }
